...
class Learner(object):
    def __init__(self, CNN_AE, input, target, batch_size=1000, epochs=1000):
        self.CNN_AE = CNN_AE
    ...
    def learn(self):
        for e in range(self.epochs) :
            for b in range(self.batch_num_total):
                self.CNN_AE.optimizer.zero_grad()
                predictions = self.CNN_AE.forward(self.current_batch)
                loss = self.CNN_AE.loss(predictions, self.current_target)
                print('epoch', str(e+1), '[ batch', str(b+1),'] - loss : ', str(loss.item()))
                loss.backward()
                self.next_batch()
                self.CNN_AE.optimizer.step()
        return self.CNN_AE